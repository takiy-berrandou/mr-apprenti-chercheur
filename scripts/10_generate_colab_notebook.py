...
config_file = Path("config/gwas_catalog_exposures.tsv")
harm_dir = Path("data/harmonised")
out_file = Path("notebooks/MR_educatif_SCAD.ipynb")
...
if config_file.exists():
    with config_file.open() as f:
        ...
        for row in reader:
            ...
            fname = f"{exposure_id}_scad.tsv"
            if (harm_dir / fname).exists():
                labels.append(label)
                file_entries.append((label, fname))

if not labels:
    labels = ["BMI"]
    file_entries = [("BMI", "bmi_scad.tsv")]

param_options = "[" + ", ".join(json.dumps(x) for x in labels) + "]"
default_label = labels[0]

file_map_lines = []
for label, fname in file_entries:
    file_map_lines.append(f'  {json.dumps(label)} = paste0(base_url, {json.dumps(fname)})')
  ...
  "cells": [
    ...
    },
    {
      ...
      "source": [
        ...
        f"exposure_choice <- {json.dumps(default_label)} #@param {param_options}\n",
        "\n",
